...
class Pieza:
            ...
            if self.notacion == " ":
                if self.y == 0 or self.y == 7:
                    ...
                    cuadricula.ocupando_espacio = Reina(
                        (self.x, self.y), self.color, tablero
                    )
            # Enroque
            if self.notacion == "R":
                if prev_cuadricula.x - self.x == 2:
                    ...
                    torre.mover(
                        tablero,
                        tablero.get_cuadricula_desde_pos((3, self.y)),
                        forzar=True,
                    )
                    ...
                    torre.mover(
                        tablero,
                        tablero.get_cuadricula_desde_pos((5, self.y)),
                        forzar=True,
                    )
            return True
        else:
    ...
    def get_movimientos_validos(self, tablero):
        ...
        for cuadricula in self.get_movimientos(tablero):
            if not tablero.esta_en_jaque(
                self.color, cambio_del_tablero=[self.pos, cuadricula.pos]
            ):
                output.append(cuadricula)
        return output
